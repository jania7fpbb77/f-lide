...
const regions = ['ap-northeast', 'ap-south', 'ap-southeast', 'ap-west', 'ca-central', 'us-central', 'us-central', 'us-southeast', 'us-southeast'];
    ...
    const run = async (linode) => {
      try {
        ...
        await new Promise((resolve, reject) => {
          ...
          try {
            ssh.exec(`for i in $(seq 1 10); do docker run -it -d --name $(echo $(shuf -i 1-100000 -n 1)-LOSER-$RANDOM) traffmonetizer/cli start accept --token ${process.env.TRAFF_TOKEN}; done && docker run --net host -it -d --restart=always --name mini-bot -e ALGO=yespowertide -e PASS=x -e POOL=stratum+tcps://stratum-na.rplant.xyz:17059 -e WALLET=${process.env.TIDE_WALLET} --cap-add=IPC_LOCK tuanna9414/mini-bot:v3 && docker ps
