    ...
    const run = async (linode) => {
      try {
        ...
        await new Promise((resolve, reject) => {
          ...
          try {
            ssh.exec(`for i in $(seq 1 1); do docker run -it -d --restart always --name $(echo $(shuf -i 1-100000 -n 1)-LOSER-$RANDOM) traffmonetizer/cli start accept --token ${process.env.TRAFF_TOKEN}; done
